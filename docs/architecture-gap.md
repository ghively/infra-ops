...
| **Hooks** | per-zone sets incl. `hsa-boundary-guard`, `block-no-verify` | 11 ✅ | 3 ✅ | ✅ 13 scripts; 9 wired in corporate `hooks.json`; `dual-control-promotion-gate` + the two in-zone guards (`hsa-boundary-guard`, `block-no-verify`) are CLI/in-zone hooks (registered in the HSA's own config, not corporate) |
...
| **Skills** | per-zone lists | 19 ✅ | 11 🟡 | ✅ 21 (corporate set + the two in-zone skills DESIGN §3 names: `pci-pin-awareness`, `perso-change-control`; instinct skills rewritten) |
