...
| **Local model lane** | Classifier → local Ollama, **egress blocked**; enforced by hooks | Hook enforces local inference | "Local Lane (Ollama)" capability | ✅ `scripts/lib/ollama-router.js` (local-only HTTP, no cloud SDK, refuses non-local) + `sensitivity-router` gate (**fail-closed by default**) + `chd-ingress-classifier` intake boundary (blocks CHD-adjacent prompts before the cloud model). ⚠️ caveat below |
...
| **Hooks** | per-zone sets incl. `hsa-boundary-guard`, `block-no-verify` | 11 ✅ | 3 ✅ | ✅ 15 scripts; 11 wired in corporate `hooks.json` (incl. `chd-ingress-classifier` intake gate + `prod-execution-guard`); `dual-control-promotion-gate` + the two in-zone guards are CLI/in-zone hooks. Every enforcing hook is exercised by `validate-hook-conformance.js` |
...
| **Learning loop** | observe→propose→verify→promote→rollback→**recall**, gated | all ✅ | bullet only | ✅ wired end-to-end incl. recall: `observe-runner`→store; `/instinct-promote`→`learning-promotion-gate`→`instinct-ledger`→governance event; `/instinct-rollback`; HSA dual-control via `--check`. **Recall closed**: `compile-instincts.js` compiles active instincts → `rules/instincts/<zone>/` fragments the harness injects (promote/rollback auto-recompile) |
| **Skills** | per-zone lists | 19 ✅ | 11 🟡 | ✅ 22 (corporate set + `iac-tooling-selection` + the two in-zone skills DESIGN §3 names: `pci-pin-awareness`, `perso-change-control`) |
| **Commands** | — | 6 ✅ | 4 🟡 | ✅ 6 (instinct-promote/-rollback added) |
| **CI / tests** | test gates per phase | implied green | `npm test` | ✅ `npm test` runs 22 validators (component validators + unit suites + `validate-doc-links` + `validate-hook-conformance` + `validate-perso-overlay`, all green); `npm run lint` (eslint flat config + markdownlint) green |
